        ...
        def add_google_style_bibtex_key(book_ids):
            ...
            for book_id in book_ids:
                ...
                if current_bibtex == '':
                    ...
                    first_author = db.get("authors").split(", ")[0].split(" ")
                    first_author_name = first_author[0]
                    if len(first_author) > 1:
                        first_author_name = first_author_name[len(first_author)-1]

                    print(db.get("pubdate"))
                    published_year = db.get("pubdate").year

                    print(db.get("title"))
                    book_title_words = db.get("title").split(" ")

                    book_title_first_words = book_title_words[0] + book_title_words[2]

                    generated_bibtex_key = (first_author_name + published_year + book_title_first_words).lower()
